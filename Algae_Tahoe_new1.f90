...
!   carbon to ug chla (wasp 1988) and set upper and lower limits.
	   ...
	   DO i = ns,1,-1
	      IF(i.eq.1)THEN
		      paro(i) =(paro(i)-paro(i)*(dexp(-et1(i)*depth(i))))/(depth(i))
	      ELSE
		      paro(i) =(paro(i)-paro(i-1))/((depth(i)-depth(i-1)))
	      ENDIF
...
!	            stop
			 ...
			 ! IF (wqual(i,15).LT.0.01) THEN
				! segmin(i,j) = 0.0
			 ! ENDIF	
			 ! IF (wqual(i,16).LT.0.01) THEN
				! segmin(i,j) = 0.0
			 ! ENDIF	
			 ! IF (wqual(i,21).LT.0.01) THEN
				! segmin(i,j) = 0.0
			 ! ENDIF	
			 ! IF (wqual(i,22).LT.0.01) THEN
				! segmin(i,j) = 0.0
			 ! ENDIF	
			 ! IF (wqual(i,29).LT.0.01) THEN
				! segmin(i,j) = 0.0
			 ! ENDIF	
...
!gbs	grow of the algae  
 temp_opt(1)=22.0d0; temp_min(1)=12.0d0; temp_max(1)=30.0d0
!   
...
!  
	   ...
	   DO i = 1,ns
	      DO j = 1,nchl
		   ...
		   ! IF(j.eq.1.and.(temp(i).le.temp_opt(j))) THEN	   
 	            ! tempdeath(j) = 2.0-dexp(-2.3*((temp(i)-temp_opt(j))/(temp_opt(j)-temp_min(j)))**2.0)	 
           ! ELSEIF(j.eq.1.and.(temp(i).gt.temp_opt(j))) THEN
 	            ! tempdeath(j) = 2.0-dexp(-2.3*((temp(i)-temp_opt(j))/(temp_max(j)-temp_opt(j)))**2.0)	 
		   ! ELSEIF(j.eq.1.and.(temp(i).eq.temp_opt(j))) THEN
				! tempdeath(j) = 1.0
           ! ENDIF	
			tempdeath(j) = 1.0
           IF((j.eq.1).and.(temp(i).le.temp_opt(j))) THEN	   
 	            tempdeath(j) = dexp(0.10d0*(temp_opt(j)-temp(i)))
           ELSEIF((j.eq.1).and.(temp(i).gt.temp_opt(j))) THEN
 	            tempdeath(j) = dexp(0.10d0*(temp()-temp_opt(j)))
           ENDIF		   
 ...
 !         	ENDIF
            ...
            IF((wqual(i,j)-death(i,j)).lt.alg_min(j)) THEN
               death(i,j) = 0.99*(wqual(i,j)-alg_min(j))
...
!  '  depmx' set in subr. mixer
	   DO i = 1,ns
	      IF (depth(i) .ge. depmx) THEN	! only within mixed layer
		      ...
		      Do j=1,nchl
!		         IF(sedalgae(j).le.alg_minalg_min(j))sedalgae(j) = alg_min(j)
