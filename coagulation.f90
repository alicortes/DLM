...
!		REAL(8) :: beta, betash, betabr, betads, change, coag, rand1, 
       ...
       totalinorg, totalorg,  time_begin, time_end,newmass,			&
...
!  
		...
		REAL(8), DIMENSION(size) :: fraclen, dd, mass, fracdim, add
...
!fraclen = (/0.125D-18, 1.0D-15, 5.65685D-15, 1.6D-11, 6.4D-11, 2.33032218D-9, 8.114653145D-9, 999999999999.0D0/)!1.25d-13, 2.50047d-13, 5.00566d-13, 99999999.0D0/)! 1.d-12, 1.3997d-9, 3.1072d-9, 6.893d-9, 1.53d-8, 2.2463d-6, 3.01576d-6, 4.0485d-6, 5.43567D-6, 1.477d-4, 2.32d-4, 3.1d-4, 999999999.D0/) 
		dd = (/0.6d-6, 1.50d-6, 2.70d-6, 5.00D-6,			&
            8.00D-6, 20.00D-6, 48.0D-6, 9999999999.0D0/) !32.0D-6, 999999999999.0D0/) ! m, 1.d-4, 1.414d-4, 2.d-4, 2.828d-4, 4.d-4, 4.757d-4, 5.657d-4, 6.727d-4, 8.d-4, 11.31d-4, 16.d-4, 20.d-4, 9999999999.D0/)
...
!  Units are number of particles/m3 at every box
		...
		DO i = 1,ns
		   DO J =1,7
		      ...
		      if(cf(i,4).ge.3.0d8) cf(i,4)=3.0d8
...
!
      ...
      ENDDO
		...
		DO i = 1, size 
			...
			ELSEIF ( i.gt.3.and.i.le.5) THEN  		
				fraclen(i) = (0.5d0*dd(i)) ** fracdim(i)
				mass(i) = (1.00D0 * inorgden(i) + 0.00D0 * orgden)	&
            *(4.0D0/3.0D0)*pi*(dd(i)/2.0D0)**3.0D0 !assigning mass and length to bins
			ELSEIF (i.gt.5.AND.i.le.size-1) THEN
					...
					DO ii = 1, (size - 1) ! mass equate  (for two similar	k==ii)
						DO j = 1, (size - 1)  ! length equate (for two similar	L==j)
							...
							ELSEIF(L.EQ.2.AND.J.EQ.2) THEN			
								coag = 0.0010D0			!0.5
							ELSEIF(L.EQ.3.AND.J.EQ.3) THEN			
								coag = 0.001D0			!0.02
							ELSEIF(L.EQ.4.AND.J.EQ.4) THEN			
								coag = 0.001D0			!0.02
							ELSEIF(L.EQ.5.AND.J.EQ.5) THEN			
								coag = 0.001D0			!0.02
...
5						CONTINUE
                  GOTO 6
!---------------------------------------------------------------------------------------------
						IF(L.eq.1.and.j.eq.1) THEN       !5 and 5
							IF(L.EQ.J) THEN
								coag=4.0d-3*DEXP(-(2.0d6/((1.0d0/dd(L))+(1.0d0/dd(j)))))*	&	!3.5d-5
          						(DLOG10(aa(ln, L,j))**2.0)*(DLOG10(aa(ln, L,j))**2.0)          				
							ELSE
								coag=0.0d0
							ENDIF
							IF(L.eq.1) THEN
								IF(aa(ln, L, L+1).gt.0.95d0*aa(ln, L, L)) coag=coag/10.0d0			   
							ELSE
								IF(aa(ln, L, L).gt.0.95d0*aa(ln, L, L-1)) coag=coag/10.0d0
							ENDIF
							IF(J.eq.1) THEN
								IF(aa(ln, J, J+1).gt.0.95d0*aa(ln, J, J)) coag=coag/10.0d0			   
							ELSE
								IF(aa(ln, J, J).gt.0.95d0*aa(ln, J, J-1)) coag=coag/10.0d0
							ENDIF							             
							IF (SD.lt.16.0d0) coag=coag/10.0d0
						ELSEIF(L.eq.2.and.j.eq.2) THEN       !5 and 5
							IF(L.EQ.J) THEN
								coag=1.0d-4*DEXP(-(2.0d6/((1.0d0/dd(L))+(1.0d0/dd(j)))))*	&	!3.5d-5
          						(DLOG10(aa(ln, L,j))**2.0)*(DLOG10(aa(ln, L,j))**2.0)          				
							ELSE
								coag=0.0d0
							ENDIF
							...
							ENDIF							             
							IF (SD.lt.16.0d0) coag=coag/10.0d0		
						ELSEIF(L.eq.3.and.j.eq.3) THEN       !5 and 5
							IF(L.EQ.J) THEN
								coag=1.0d-5*DEXP(-(2.0d6/((1.0d0/dd(L))+(1.0d0/dd(j)))))*	&	!3.5d-5
          						(DLOG10(aa(ln, L,j))**2.0)*(DLOG10(aa(ln, L,j))**2.0)          				
							ELSE
								coag=0.0d0
							ENDIF
							...
							ENDIF							             
							IF (SD.lt.16.0d0) coag=coag/10.0d0	
					  ELSEIF(L.eq.4.and.j.eq.4) THEN       !5 and 5
							IF(L.EQ.J) THEN
								coag=1.0d-6*DEXP(-(2.0d6/((1.0d0/dd(L))+(1.0d0/dd(j)))))*	&	!3.5d-5
          						(DLOG10(aa(ln, L,j))**2.0)*(DLOG10(aa(ln, L,j))**2.0)          				
							ELSE
								coag=0.0d0
							ENDIF
							...
							ENDIF							             
							IF (SD.lt.16.0d0) coag=coag/10.0d0		
					ELSEIF(L.eq.5.and.j.eq.5) THEN       !5 and 5
							IF(L.EQ.J) THEN
								coag=1.0d-7*DEXP(-(2.0d6/((1.0d0/dd(L))+(1.0d0/dd(j)))))*	&	!3.5d-5
          						(DLOG10(aa(ln, L,j))**2.0)*(DLOG10(aa(ln, L,j))**2.0)          				
							ELSE
								coag=0.0d0
							ENDIF
...
!-------------------------------------------------------------------------------------		 
 6					CONTINUE
 						IF(L.le.5.and.J.le.5) THEN
						   coag=0.015
						   IF (SD.lt.15.0d0) coag=coag/10.0d0
						ELSE
						   coag=0.0
						ENDIF
							Betash = (MIN((dd(L)/2 + b(ln,k,L))**(1.0D0/fracdim(L)),						&
					                   (dd(j)/2 + b(ln,ii,j))** (1.0D0/fracdim(j))) /					&
					               MAX((dd(L)/2 + b(ln,k,L)) ** (1.0D0/fracdim(L)),					&
					                   (dd(j)/2 + b(ln,ii,j)) ** (1.0D0/fracdim(j)))					&
											  ** 0.88)* 1.3D0 * ((eps(ln) / kinvis(ln)) ** 0.5D0) *		&
					                  ((dd(L)/2 + (b(ln,k,L)/2)**(1.0D0/fracdim(L))) +				&
											 (dd(j)/2 + ((b(ln,ii,j)/2)**( 1.0D0/fracdim(j))))) ** 3
							IF (((gr*(mass(k)+c(ln,k,L)))/(6.0D0*pi*dyvis(ln)								&
								* ( dd(L) / 2 + ( b(ln,k,L) / 2 ) ** ( 1.0D0/fracdim(L))))				&
								 - (gr * ( mass(ii) + c(ln,ii,j)))								            &
								/ ( 6.0D0 * pi * dyvis(ln) * ( dd(j) / 2 +									&
								( b(ln,ii,j) / 2 ) ** ( 1.0D0/fracdim(j) ) ) ) *							&
								( ( dd(L) / 2 + ( b(ln,k,L)/2) ** ( 1.0D0/									&
								fracdim(L))) - (dd(j) / 2 + (b(ln,ii,j)/2)									&
								** ( 1.0D0/fracdim(j))))) .ge. 0 ) THEN  
								
								Betads = ( MIN( (dd(L)/2 + b(ln,k,L)) **										&
     									(1.0D0/fracdim(L)) , ( dd(j) / 2 + b(ln,ii,j))						&
     									** (1.0D0/fracdim(j)) ) / MAX( (dd(L)/2 +								&
     									b(ln,k,L)) ** (1.0D0/fracdim(L)) , ( dd(j)/							&
     									2 + b(ln,ii,j)) ** (1.0D0/fracdim(j))) ** 0.984)					&
				                  * ABS(pi * ( ( (dd(L) / 2 + ( b(ln,k,L)/2)							&
				                  ** ( 1.0D0/fracdim(L) )  + dd(j) / 2 +									&
				                  (b(ln,ii,j)/2) ** (1.0D0/fracdim(j))))**2.0D0)*						&
				                  ((gr * (mass(k) + c(ln,k,L)))/(6.0D0 * pi*							&
				                  dyvis(ln) * ( dd(L) / 2 + (b(ln,k,L)/2) **							&
				                  (1.0D0/fracdim(L)))) - (gr*(mass(ii) +									&
				                  c(ln,ii,j))) / (6.0D0 * pi * dyvis(ln) * (dd(j)						&
				                  / 2 + (b(ln,ii,j)/2) ** (1.0D0/fracdim(j))))))
							ELSE 
								Betads = ABS(pi*(((dd(L)/2+(b(ln,k,L) / 2 )**								&
                       (1.0D0/fracdim(L) )  + dd(j) / 2 +												&
                       (b(ln,ii,j)/2)**(1.0D0/fracdim(j))))** 2.0D0 )								&
                       * ((gr * ( mass(k) + c(ln,k,L)))/(6.0D0 * pi									&
                       * dyvis(ln) * ( dd(L) / 2 + (b(ln,k,L)/2)**									&
                       ...
                       / 2 + ( b(ln,ii,j)/2) ** (1.0D0/fracdim(j))))))
							END IF
							Betabr = 4.0D0 * pi * ( kboltz * (temp(ln)+273.15) *							&
                        (( 6.0D0 * pi *dyvis(ln) * ( dd(L) / 2 +										&
                        ( b(ln,k,L) / 2 )**(1.0D0/fracdim(L)))) **									&
                         -1.0D0) + kboltz *(temp(ln)+273.15)*											&
                        (( 6.0D0 * pi * dyvis(ln) * ( dd(j) / 2 +										&
     							(b(ln,ii,j)/2)**(1.0D0/fracdim(j))))** -1.0D0))*                     &
     							((dd(L)/2 + (b(ln,k,L)/2)**(1.0D0/fracdim(L))) +				         &
								 (dd(j)/2 + ((b(ln,ii,j)/2)**( 1.0D0/fracdim(j)))))   
							beta = betash + betads + betabr 
!     check	 		  
							IF ( k == ii .AND. L == j) THEN
								change=0.5*coag*beta*aa(ln,k,L)*aa(ln,ii,j)*(1/count3)**2							  
							ELSE
								change=1.0d0*coag*beta*aa(ln,k,L)*aa(ln,ii,j)*(1/count3)**2 ! compared to 1.0  							                     
							ENDIF		
!	                  print*, ln,k,ii,l,j,betash, betads, betabr,change
!							pause   
							IF(k == ii .AND. L == j .AND. change >= aa(ln,ii,j)/2)THEN !Dav 18/8/04 accounting for too much change
								change = aa(ln,ii,j) / 2	          
								bbb = bbb + 1
...
! remember to add condition about what happens if the 2 particles exceed the matrix	
							IF (((mass(k) + mass(ii) + c(ln,k,L) + c(ln,ii,j))>=					&
     							MAX( mass(k+1), mass(ii+1) ) ) .OR. ( ( fraclen(L) +				&
     							fraclen(j) + b(ln,k,L)+b(ln,ii,j))>= MAX(fraclen(L+1),fraclen(j+1)))) THEN 		
			 

								DO m1 = 1 , ( size - 1 )
									IF ((mass(k) + mass(ii) + c(ln,k,L) + c(ln,ii,j)) >= mass(m1)) THEN
										count1 = count1 + 1.
									ENDIF 				
								ENDDO !m1

								DO n1 = 1 , ( size - 1 )
									IF ((fraclen(L) + fraclen(j) + b(ln,k,L) + b(ln,ii,j)) .ge. fraclen(n1)) THEN
										count2 = count2 + 1
									ENDIF 
								ENDDO !n1
								IF(count1.le.0) THEN
